...
总结：
```
promise 三种状态 Pending(进行中)、Resolved(已完成，又称Fulfilled)，Rejected(已失败)。
围绕这三种状态有两个调函数，resolve(Pending->Resolved),reject(Pending->Rejected)
状态一旦改变，就不会再变
新建
1、new Promise(function(resolve,reject){
    ...
    if(true)
        ...
        reject();
}).then(function(){//resolve回调
})//reject回调（可选，通常直接用catch）
2、Promise.resolve();
3、Promise.reject();
then方法返回的是一个新的Promise实例（注意，不是原来那个Promise实例）。因此可以采用链式写法，即then方法后面再调用另一个then方法。
catch方法
...
Promise.all([p1,p2,p3]).then(function(datas){
          数组中都Resolved(datas数组结果)或有一个Rejected(datas代表失败的)
})
