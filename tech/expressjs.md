...
作为 HTTP 协议的组成部分，各个框架都有请求和响应对应的类，在处理请求时，创建这些类的实例并注入到调用方（即我们）的 Router handler, Middleware 中。下面是对 Express 中请求和响应的常用 API 说明。

#### Request

`req.body`: 获取 Request Body，必须在使用了 body-parser 或者 multer 后使用，默认在 `app.js` 中已经配置了 body-parser。

`req.params`: 获取 URI Parameter，当占位符是 `:name` 的形式时，该属性是对象，是正则表达式时是数组。

`req.query`: 获取 Query String，

`req.cookie`: 获取请求发送的 Cookie，必须在 cookie-parser 后使用，已在 `app.js` 中配置了 cookie-parser。
